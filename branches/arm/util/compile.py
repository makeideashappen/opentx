...
def generate_c9x_list(id, hexes, extension, stamp, board):
    f = file("../../companion9x/src/open9x-%s-binaries.cpp" % id, "w")
    f.write('#include "open9xinterface.h"')
    f.write('void include_open9x_%s_binaries(FirmwareInfo * open9x) {' % id)
    for hex in hexes:
...
if __name__ == "__main__":
    ...
    if platform.system() == "Windows":
        ...
        generate_c9x_list("arm", hexes, "bin", "OPEN9X_ARM_STAMP", "BOARD_ERSKY9X")
        ...
        if not "v4" in sys.argv:
            ...
            generate_c9x_list("stock", hexes, "hex", "OPEN9X_STAMP", "BOARD_STOCK")
        ...
        if not "stock" in sys.argv:
            ...
            generate_c9x_list("v4", hexes, "hex", "OPEN9X_STAMP", "BOARD_GRUVIN9X")
            upload(hexes, "hex", stamp)
